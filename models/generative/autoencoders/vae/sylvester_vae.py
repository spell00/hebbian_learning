...
class SylvesterVAE(VariationalAutoencoder):
    def __init__(self, flavour, z_dims, h_dims, auxiliary, n_flows, a_dim, kernels=None, padding=None,
                 num_elements=None, hebb_layers=False,
                 dropout=0.5):
        super(SylvesterVAE, self).__init__(flavour, z_dims, h_dims, auxiliary, n_flows, kernels, padding, hebb_layers=hebb_layers,
                                           dropout=dropout)
        ...
        if self.flow_type == "o-sylvester":
            ...
            if self.auxiliary:
                if len(self.a_dims) > 1:
                    ...
                    self.amor_q_a = nn.Linear(self.h_dims[-1], self.n_flows * self.a_dim * self.num_elements)
    ...
    def forward(self, x, y=torch.Tensor([]).cuda(), a=torch.Tensor([]).cuda(), k=0, auxiliary=False):
        ...
        (z_mu, z_var, r1, r2, q, b), x, z_q = self.encode(torch.cat([x, y], 1), auxiliary=auxiliary)
    ...
    def run_sylvester(self, x, y=torch.Tensor([]).cuda(), a=torch.Tensor([]).cuda(), k=0, auxiliary=True):
        """
        Forward pass with orthogonal sylvester flows for the transformation z_0 -> z_1 -> ... -> z_k.
        Log determinant is computed as log_det_j = N E_q_z0[\sum_k log |det dz_k/dz_k-1| ].
        """
