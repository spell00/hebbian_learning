...
from models.neural_net import neural_net
...
def test_mlp():
    ...
    ae = ae()
    ae.set_configs(home_path=g.home_path, results_folder=g.results_folder, data_folder=g.data_folder,
                      destination_folder=g.destination_folder, dataset_name=g.meta_filename,
                      meta_destination_folder=g.meta_destination_folder,csv_filename="csv_loggers")
    ae.import_dataframe(g.meta_df)
    ae.set_data()
    input_size = len(list(g.meta_df.index))
    ae.define_configurations(early_stopping=100, warmup=10, z1_size=2, z2_size=2, input_size=input_size, model_name="vae")
    ae.make_data_loaders(batch_size=32,test_batch_size=32)
    ...
    mlp = mlp_keras(home_path=g.home_path, results_folder=g.results_folder, data_folder=g.data_folder,
                    destination_folder=g.destination_folder, dataset_name=g.meta_filename,
